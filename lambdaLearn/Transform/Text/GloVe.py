...
class Glove(Transformer):
    def __init__(
        ...
        :param dim: The dimension of the word vector.
        ...
        self.vec.vectors = torch.cat(
            [
                self.vec.vectors,
                self.pad_init(torch.Tensor(self.vec.vectors.shape[1])).unsqueeze(0),
                self.unk_init(torch.Tensor(self.vec.vectors.shape[1])).unsqueeze(0),
            ],
            dim=0,
        )
