...
class Discriminator(nn.Module):
    def __init__(
        self,
        dim_in=28**2,
        hidden_dim=[1000, 500, 250, 250, 250],
    ...
    def forward(self, x):
        ...
        noise = (
            torch.randn(x.size()).to(self.device) * self.noise_level[0]
            if self.training
        ...
        for i in range(len(self.layers)):
            ...
            noise = (
                torch.randn(x_f.size()).to(self.device) * self.noise_level[i + 1]
                if self.training
                else torch.Tensor([0]).to(self.device)
            )
...
class Generator(nn.Module):
    def __init__(
        ...
        dim_in=28**2,
        hidden_dim=[500, 500],
        activations=[nn.Softplus(), nn.Softplus(), nn.Softplus()],
        dim_z=100,
        device="cpu",
    ):
    ...
    def forward(self, batch_size=10, z=None):
        z = (
            Variable(
                torch.rand(batch_size, self.dim_z),
                requires_grad=False,
