...
import lambdaLearn.Config.Foster as config
...
class Foster(DeepModelMixinCIL):
    def __init__(self):
        ...
        self.is_teacher_wa = config.is_teacher_wa
        self.is_student_wa = config.is_student_wa
        self.lambda_okd = config.lambda_okd
        self.wa_value = config.wa_value
        self.oofc = config.oofc.lower()
    ...
    def incremental_train(self, data_manager: CILDataset):
        ...
        self.dataset = data_manager.dataset_name
        self._cur_task += 1
        if self._cur_task > 1:
    ...
    def _train(self, train_loader, test_loader):
        ...
        if self._cur_task == 0:
            ...
            scheduler = optim.lr_scheduler.CosineAnnealingLR(
                optimizer=optimizer, T_max=config.boosting_epochs
            )
            if self.oofc == "az":
    ...
    def _init_train(self, train_loader, test_loader, optimizer, scheduler):
        prog_bar = tqdm(range(config.init_epochs))
    ...
    def _feature_boosting(self, train_loader, test_loader, optimizer, scheduler):
        prog_bar = tqdm(range(config.boosting_epochs))
        for _, epoch in enumerate(prog_bar):
            self.train()
            losses = 0.0
            losses_clf = 0.0
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                loss_kd = self.lambda_okd * KD_loss(
                    logits[:, : self._known_classes], old_logits, config.T
                )
    ...
    def _feature_compression(self, train_loader, test_loader):
        self._snet = FOSTERNet(config.model_name, config.seed)
        ...
        scheduler = optim.lr_scheduler.CosineAnnealingLR(
            optimizer=optimizer, T_max=config.compression_epochs
        )
        self._network.eval()
        prog_bar = tqdm(range(config.compression_epochs))
        for _, epoch in enumerate(prog_bar):
            self._snet.train()
            losses = 0.0
            correct, total = 0, 0
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                loss_dark = self.BKD(dark_logits, logits, config.T)
                loss = loss_dark
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
