...
class BEEFISO(DeepModelMixinCIL):
    def __init__(self):
        ...
        self._snet = None
        self.logits_alignment = config.logits_alignment
        self.val_loader = None
        self.reduce_batch_size = config.reduce_batch_size
        self.random = config.random if hasattr(config, "random") else None
        self.imbalance = config.imbalance if hasattr(config, "imbalance") else None
    ...
    def after_task(self):
        ...
        if self.reduce_batch_size:
            if self._cur_task == 0:
                ...
                config.batch_size = config.batch_size * (self._cur_task + 1) // (self._cur_task + 2)
    ...
    def incremental_train(self, data_manager: CILDataset):
        ...
        self._cur_task += 1
        if self._cur_task > 1 and config.is_compress:
        ...
        if self._cur_task > 0:
            ...
            self.val_loader = DataLoader(
                val_dset, batch_size=config.batch_size, shuffle=True, num_workers=config.num_workers,
                pin_memory=True)
    ...
    def _train(self, train_loader, test_loader, val_loader=None):
        ...
        if self._cur_task == 0:
            ...
            scheduler = optim.lr_scheduler.CosineAnnealingLR(
                optimizer=optimizer, T_max=config.init_epochs
            )
            self.epochs = config.init_epochs
            ...
            scheduler = optim.lr_scheduler.CosineAnnealingLR(
                optimizer=optimizer, T_max=config.expansion_epochs
            )

            self.epochs = config.expansion_epochs
            ...
            self.state = "fusion"
            self.epochs = config.fusion_epochs
            ...
            for n, p in self._network.named_parameters():
                if p.requires_grad:
                    print(n)
            scheduler = optim.lr_scheduler.CosineAnnealingLR(
                optimizer=optimizer, T_max=config.fusion_epochs
            )
    ...
    def _init_train(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                loss_en = config.energy_weight * self.get_energy_loss(inputs, targets, targets)
    ...
    def _expansion(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                loss_en = config.energy_weight * self.get_energy_loss(inputs, targets, pseudo_targets)
    ...
    def _reduce_exemplar_imbalance(self, data_manager, m, random, imbalance):
        ...
        for class_idx in range(self._known_classes):
            ...
            idx_loader = DataLoader(idx_dataset, batch_size=config.batch_size, shuffle=False, num_workers=4)
            vectors, _ = self._extract_vectors(idx_loader)
            vectors = (vectors.T / (np.linalg.norm(vectors.T, axis=0) + config.EPSILON)).T
    ...
    def _construct_exemplar_imbalance(self, data_manager, m, random=False, imbalance=None):
        ...
        if random:
            ...
            selected_indices = np.random.choice(a=list(range(len(data))), size=m * increment, replace=False)
    ...
    def _construct_exemplar_unified_imbalance(self, data_manager, m, random, imbalance):
        ...
        for class_idx in range(self._known_classes):
            ...
            class_loader = DataLoader(class_dset, batch_size=config.batch_size, shuffle=False, num_workers=4)
        ...
        if random:
            ...
            for class_idx in range(self._known_classes, self._total_classes):
                ...
                exemplar_dset = data_manager.get_dataset([], source='train', mode='test',
                                                         appendent=(selected_exemplars, exemplar_targets))
                exemplar_loader = DataLoader(exemplar_dset, batch_size=config.batch_size, shuffle=False,
                                             num_workers=4)
