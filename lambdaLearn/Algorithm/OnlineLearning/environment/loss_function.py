...
class LossFunction(ABC):
    ...
    Users can define their loss functions by inheriting from this class and override the method :meth:`~ environment.loss_function.LossFunction.__getitem__`.
    """
...
class CustomFunction(LossFunction):
    """This class contains user defined loss functions.

    Args:
        feature (numpy.ndarray): Features of the environment.
        label (numpy.ndarray): Labels of the environment.
        f (Callable[[np.ndarray], float]): User defined loss functions.
    ...
    def func(self, x, t):
        loss = self.f(x, self.feature[t], self.label[t])
        return loss