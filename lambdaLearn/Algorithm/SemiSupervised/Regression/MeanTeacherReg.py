...
class MeanTeacherReg(DeepModelMixin, InductiveEstimator, RegressorMixin):
    def __init__(
        ...
        """
        :param ema_decay: Update weights for the exponential moving average.
        :param warmup: The end position of warmup. For example, num_it_total is 100 and warmup is 0.4, then warmup is performed in the first 40 iterations.
        :param lambda_u: The weight of unsupervised loss.
        :param dim_in: the dim of the instances.
        """
    ...
    def start_fit(self):
        self.dim_in = self.dim_in if self.dim_in is not None else self._train_dataset.labeled_dataset.X.shape[-1]
    ...
    def get_loss(self, train_result, *args, **kwargs):
        ...
        _warmup = float(np.clip((self.it_total) / (self.warmup * self.num_it_total), 0.0, 1.0))
